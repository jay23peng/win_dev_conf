...
start "%VIM_EXE_DIR%\gvim.exe" --servername GVIM2 %VIMARGS%
goto eof

:nofork
start /w "%VIM_EXE_DIR%\gvim.exe" --servername GVIM2 %VIMARGS%
goto eof

:ntaction
rem for WinNT we can use %*
if .%VIMNOFORK%==.1 goto noforknt
start "dummy" /b "%VIM_EXE_DIR%\gvim.exe" --servername GVIM2 %*
goto eof

:noforknt
start "dummy" /b /wait "%VIM_EXE_DIR%\gvim.exe" --servername GVIM2 %*
