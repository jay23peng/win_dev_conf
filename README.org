...
*** Browser shortcut
    ...
    #+begin_src
    # split window
    Cmd + Shift + '   => Tile Vertically (Left and Right)
    Cmd + Shift + '   => Tile Horizontally (Up and Down)
    Cmd + Shift + m  => Untile window (max current selected window)
    ...
    Cmd + Shift + c  => Switch dark/light mode on current page (Midnight Lizard)
    Cmd + Shift + n   => move current tab to new window
